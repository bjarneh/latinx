...
func (e UnicodeError) String() string {
    if int(e) == 1 {
...
func (e UnknownByteError) String() string {
    return string(e)
}
...
func (e UnknownRuneError) String() string {
    return string(e)
}
...
func (c *Converter) Encode(utf_8 []byte) (latin []byte, success int, err os.Error) {

    var ok bool
    var latinByte byte
    var offset, rune, size int
    var errmsg string
    ...
    for offset < len(utf_8) {
        ...
        rune, size = utf8.DecodeRune(utf_8[offset:])
        ...
        } else if rune < utf8.RuneSelf {
            buf.WriteByte(utf_8[offset])
            offset++
        } else {
            latinByte, ok = c.utf8ToLatin[rune]
            if !ok {
                errmsg = fmt.Sprintf("undefined: 0x%X in %s", rune, c.id)
...
func (c *Converter) Decode(latin []byte) (utf_8 []byte, err os.Error) {
...
func Encode(charset int, utf_8 []byte) (latin []byte, success int, err os.Error) {
    return converters[charset].Encode(utf_8)
}

// Convert a ISO-8859 encoded slice to a UTF-8 encoded slice
func Decode(charset int, latin []byte) (utf_8 []byte, err os.Error) {
